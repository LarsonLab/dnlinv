...
if [ ! -e data/knee-fully-sampled-reference.cfl ] ; then
    bart fft -u -i $(bart bitmask 1 2) data/full data/tmp_knee_full_ifft
...
do
	...
	echo "DIP ${US}"
    python $DNLINV_PATH/measure_error.py data/knee-fully-sampled-reference reco_DIP/dip_${US}/rsos_multicoil_estimate

    echo "ENLIVE ${US}"
    python $DNLINV_PATH/measure_error.py data/knee-fully-sampled-reference reco_ENLIVE/r_mm_${US}

#    echo "SAKE ${US}"
#    python $DNLINV_PATH/measure_error.py data/knee-fully-sampled-reference reco_SAKE/r_sake_${US}
