...
def reconstruct(hparams):
    ...
    if hparams.debug:
        ...
        try:
            ...
            my_model.g.max_likelihood = False  # 2D ABLATION MODIFICATION - to force proper MC inference in the model
            my_model.g.z_var = nn.Parameter(torch.tensor(my_model.g.dip_stdev))  # 2D ABLATION MODIFICATION - to force proper MC inference in the model
            my_model.g.z_var.requires_grad = False
            ...
            # Do inference
            if False: # hparams.maximum_likelihood:  # 2D ABLATION MODIFICATION - Force MC-inference even in DIP/maximum likelihood mode
                z, x, mps = my_model.g()
