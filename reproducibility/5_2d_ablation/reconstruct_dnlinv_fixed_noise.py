        ...
        if hasattr(my_model.g, 'calculate_elbo_entropy'):
            ...
            if hparams.noise_covariance_estimate:
                optim = optimizer_to_use([{'params': net_params_no_var, 'weight_decay': hparams.weight_decay},
                                          ...
                                          # {'params': my_model.L_log_diag},  # 2D ABLATION MODIFICATION Commented-out to keep noise covariance fixed
                                          # {'params': my_model.L_lower},  # 2D ABLATION MODIFICATION Commented-out to keep noise covariance fixed
                                          {'params': my_model.scale_factor}],
                                         lr=lr)
            else:
                optim = optimizer_to_use([{'params': net_params_no_var, 'weight_decay': hparams.weight_decay},
                                          {'params': net_params_var_only},
                                          # {'params': my_model.log_noise_sigma},  # 2D ABLATION MODIFICATION Commented-out to keep noise variance fixed
                                          {'params': my_model.scale_factor}],
                                         lr=lr)
        else:
            if hparams.noise_covariance_estimate:
                ...
                optim = optimizer_to_use([{'params': my_model.g.parameters(), 'weight_decay': hparams.weight_decay},
                                          # {'params': my_model.log_noise_sigma},  # 2D ABLATION MODIFICATION Commented-out to keep noise covariance fixed
                                          {'params': my_model.scale_factor}],
                                         lr=lr)
